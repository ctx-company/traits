...
let activeConfigFrame: ConfigFrame | undefined;
...
export function isConfigFrameActive(): boolean {
  return activeConfigFrame !== undefined;
}

export function markLegacyDefineConfigTopLevel(): void {
  legacyDefineConfigTopLevelSeen = true;
}

export function openConfigFrame(): void {
...
export function closeConfigFrame(): ConfigFrame {
  const frame = activeConfigFrame;
  if (frame === undefined) throw new Error("evaluateConfigFunction: no active config function to close");
...
export function currentConfigFrame(caller: string): ConfigFrame {
  if (activeConfigFrame === undefined) {
    throw new Error(
      `${caller} called outside the config build — config.ts must default-export a function of define* calls`,
    );
  }
...
export function registerSingleton(caller: string, table: ConfigTable, value: unknown): void {
...
export function registerKeyed(caller: string, table: ConfigTable, key: string, value: unknown): void {
  const frame = currentConfigFrame(caller);
