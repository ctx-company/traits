 ...
 */
export type PromptRegistrarOptions = Omit<
...
export type ParallelSequenceFields = Omit<
  SequenceCommonFields,
  | "agent"
  | "input"
  | "output"
...
export type BranchSequenceFields = Omit<
  SequenceCommonFields,
  | "agent"
  | "input"
  | "output"
 ...
 */
export type GateBranchFields = Omit<BranchSequenceFields, "if" | "then" | "otherwise"> & {
...
export interface SequenceFunction {
  ...
  forEach<Item = unknown>(
    ...
    ) => { readonly body: readonly SequenceHandle[] } & Omit<
      ForEachSequenceFields,
...
function sequenceGate(producer: SequenceHandle, options: GateOptions): SequenceHandle {
  ...
  const ask = sequence.ask(`${id}-gate-ask`, {
    prompt: input.prompt`Review the current proposal:\n${options.proposal}\nSubmit {action: approve|edit|reject, edit?: string}.`,
    when: options.when,
    output: decision,
  });
...
export const sequence: SequenceFunction = {
  ...
  loop: (id: string, fields: Omit<LoopSequenceFields, "id" | "kind">): SequenceHandle => {
    ...
    const abortIf =
      fields.abortIf === undefined ? undefined : lowerCheckGuard(fields.abortIf, `sequence.${id}.abortIf`);
...
function sequenceOf(fields: SequenceFields): SequenceHandle {
  ...
  const command =
    ...
    fields.argv !== undefined ||
    rawFields.argvFrom !== undefined ||
  ...
  if (isSequenceKind(fields, kind, "loop")) {
    ...
    if (loop.iterations !== undefined && typeof loop.iterations !== "number" && loop.maxIterations !== undefined) {
  ...
  const input =
    kind === "prompt" || kind === "ask"
      ...
      : isSequenceKind(fields, kind, "command")
        ? commandInput(commandDeps, commandArgvFields, fields.argvFrom, fields.executableDigestFrom)
        : kind === "check"
  ...
  const otherwiseArm =
    branch?.otherwise === undefined ? undefined : normalizeBranchArm(fields.id, "otherwise", branch.otherwise);
  const inlineThen = branch !== undefined && Array.isArray(branch.then) ? branch.then : undefined;
  const inlineOtherwise =
    branch?.otherwise !== undefined && Array.isArray(branch.otherwise) ? branch.otherwise : undefined;
  const inlineBranchArms =
    inlineThen === undefined && inlineOtherwise === undefined
      ? undefined
      : {
  ...
  const inlineBody =
    isSequenceKind(fields, kind, "loop") || isSequenceKind(fields, kind, "for-each")
  ...
  if (kind === "command" || kind === "check") {
    ...
    canonical.command = {
      argv:
        commandFields?.argvFrom !== undefined
          ? undefined
          : fields.cmd === undefined
            ? validateArgv(
      ...
      "executable-digest-from":
        commandFields?.executableDigestFrom === undefined
          ? undefined
  ...
  if (kind === "sequence" || kind === "loop" || kind === "for-each") {
    canonical.sequence =
      fields.sequence === undefined ? undefined : refText(fields.sequence, `sequence.${fields.id}.sequence`);
  }
  if (kind === "branch") {
    canonical.sequence = thenArm === undefined ? undefined : refText(thenArm, `sequence.${fields.id}.then`);
    ...
    canonical.otherwise =
      otherwiseArm === undefined ? undefined : refText(otherwiseArm, `sequence.${fields.id}.otherwise`);
  }
  if ((kind === "prompt" || kind === "command") && fields.onFailure !== undefined) {
    canonical["on-failure"] = normalizeFailureTarget(fields.onFailure, `sequence.${fields.id}.onFailure`);
  }
  if (isSequenceKind(fields, kind, "loop")) {
    const loop = fields;
    ...
    canonical["max-iterations-from"] =
      iterations === undefined || typeof iterations === "number"
        ? undefined
    ...
    canonical["on-abort"] =
      loop.onAbort === undefined ? undefined : normalizeExhaustionTarget(loop.onAbort, `sequence.${fields.id}.onAbort`);
  }
  if (isSequenceKind(fields, kind, "for-each")) {
    ...
    canonical["on-failure"] =
      each.onFailure === undefined
        ? undefined
        : normalizeFailureTarget(each.onFailure, `sequence.${fields.id}.onFailure`);
  ...
  if (isSequenceKind(fields, kind, "parallel")) {
    ...
    canonical["branch-failure"] = normalizeBranchFailure(parallel.branchFailure, fields.id);
    canonical["on-failure"] =
      parallel.onFailure === undefined
        ? undefined
        : normalizeFailureTarget(parallel.onFailure, `sequence.${fields.id}.onFailure`);
  }
  ...
  const controlFields = fields as Omit<Partial<LoopSequenceFields>, "onFailure"> & Partial<ForEachSequenceFields>;
...
function materializeInlineSequenceItem(item: SequenceHandle, scope: SequenceScope): SequenceHandle {
  ...
  const thenArm =
    inline?.trueArm === undefined
      ? undefined
      : sequenceLinear(thenId, inline.trueArm, { generatedBranchArm: { baseId: thenId } });
  const otherwiseArm =
    inline?.falseArm === undefined
      ? undefined
      : sequenceLinear(otherwiseId, inline.falseArm, { generatedBranchArm: { baseId: otherwiseId } });
  ...
  const materialized = compact({
    ...
    otherwise: otherwiseArm === undefined ? item.otherwise : refText(otherwiseArm, `sequence.${stepId}.otherwise`),
  });
  const {
...
export function normalizeMaterializedSequenceItem(item: SequenceHandle): JsonObject {
  ...
  return bindings === undefined ? normalized : withMeta(normalized, { generatedBranchArmBindings: bindings });
}

function generatedBranchArmId(scope: SequenceScope, branchId: string, arm: "then" | "otherwise"): string {
...
function flattenOutputItems(
  ...
  return items.flatMap((item) => {
    ...
    return content.refs.map(
      (ref) => (optionalRefs.has(ref) ? { slot: ref, optional: true } : ref) as unknown as SequenceOutputValue, // audited-unknown-cast: ref string / optional marker are not branded handles, see comment above
    );
  });
}
function outputList(
...
function promptInput(
  ...
  const inferred = (promptMeta?.refs ?? []).filter(
    (ref) => ref.startsWith("port:") || ref.startsWith("slot:") || ref.startsWith("resource:"),
  );
...
function isCommandTemplateValue(value: unknown): value is CommandTemplateValue {
  return (
    typeof value === "object" && value !== null && (value as { readonly kind?: string }).kind === "cdk-command-template"
  );
}
 ...
 */
function mergeCommandDeps(legacy: SequenceFields["input"], include: SequenceFields["input"]): SequenceFields["input"] {
...
function commandInput(
  ...
  const explicitRefs = explicit.map(inputItemRefText);
  const inferred = (argv ?? [])
    .flatMap((item) => (typeof item === "string" ? scanInterpolationRefs(item) : [metaOf(item)?.ref ?? ""]))
    .filter(
      (ref) =>
        ref.startsWith("slot:") || ref.startsWith("port:") || ref.startsWith("setting:") || ref.startsWith("resource:"),
    );
...
function normalizeProjectProjections(fields: ProjectSequenceFields): NormalizedProjectProjection[] {
  ...
  return fields.projections.map((projection, index) => {
    ...
    const handleOperation =
      declaration !== undefined ? normalizeProjectOperation(declaration.operation, `${path}.destination`) : undefined;
...
function bindDefaultOutput(value: JsonValue, path: readonly (string | number)[], output: string): JsonValue {
  if (path.length === 0) {
    return value !== null && typeof value === "object" && !Array.isArray(value) ? compact({ ...value, output }) : value;
  }
...
function normalizeParallelJoin(value: ParallelJoinOption | undefined, id: string): CanonicalJoinPolicy | undefined {
  if (value === undefined) return undefined;
...
export function validateNoDuplicateTitles(items: readonly SequenceHandle[], scopeLabel: string): void {
  ...
  for (const item of items) {
    ...
    if (existing !== undefined) {
      throw new Error(
        `${scopeLabel}: steps titled ${JSON.stringify(existing)} and ${JSON.stringify(title)} both derive id ${JSON.stringify(
          id,
        )}`,
      );
    }
