...
**Status:** filed, not scheduled (owner call 2026-08-04 — real and wanted, but its determinism story must be settled before it ships) · **Depends on:** 0090 (the `[[mcp.toolset]]` layer) · **Raised:** 2026-08-04 (owner design session; decisions in this file are the contract — they were settled deliberately, do not re-open them without a concrete contradiction)
...
A `ask` (0087) is **structural**: the trait author decides that at *this* point a question is
...
- **A grant names a toolset, and the toolset is 0090's.** That layer is the one thing that must be
  true from the start: `[[mcp.server]]` declares the connection once, `[[mcp.toolset]]` names a slice
  of reach, and a slice is granted to a seat here or bound to a peer there. Two configs describing the
  same stdio command with different environments is a bug waiting for a Friday, and one shared unit of
  reach is also what a policy layer needs to name.
- **Grants are explicit and per-role.** Named toolsets per seat. No ambient-everything, no inheritance
  from a global set — a seat's reach should be readable in one table.
...
- A grant is a capability, and the model chooses when to exercise it. That is a materially weaker
  guarantee than a declared ask, and any surface that reports what a run reached out to must
...
An agent role can be granted named toolsets shared with 0090; the host performs every
...
than incidental; and a run's report distinguishes model-chosen tool use from declared asks.
