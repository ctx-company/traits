...
export const readinessStatus = schema.enum("optimize-readiness-status", ["ready", "abort"], {
...
export const runResultStatus = schema.enum("optimize-run-result-status", ["ok", "error"], {
...
export const resultStatus = schema.enum("optimize-result-status", ["ok", "error"], {
...
export const aggregateResult = schema.object(
  "optimize-aggregate-result",
  {
    ...
    metric: schema.field(schema.number(), {
      description:
        "Median lower-is-better metric across the benchmark-runs invocations — an actually-measured value, never a fabricated average.",
    }),
...
export const summaryStatus = schema.enum("optimize-summary-status", ["completed", "aborted"], {
...
export const decisionStatus = schema.enum(
  ...
  {
    description:
      "Deterministic disposition of one round. review-rejected is benchmark-only — experiment rounds never reach it.",
  },
);
...
export const summaryRow = schema.object(
  "optimize-summary-row",
  {
    measurement: schema.field(aggregateResult, {
      required: false,
      description:
        "The trusted N-run aggregate measurement; absent for a review-rejected round, which is never measured.",
    }),
...
export const summarySchema = schema.object(
  "optimize-summary",
  {
    status: schema.field(summaryStatus, {
      description:
        "Completed once the target is reached or the iteration cap is exhausted. aborted only from preflight/baseline gating.",
    }),
...
export const reviewStatusSchema = schema.enum("optimize-benchmark-review-status", ["approved", "revise"], {
  ...
  return schema.object(
    "optimize-review-verdict",
    {
      status: schema.field(reviewStatusSchema, {
        description:
          "approved when the candidate is behavior-preserving and introduces no new smell; revise otherwise. An unapproved candidate is reverted, never repaired.",
      }),
    ...
    {
      description:
        "Typed single-pass review verdict for one implemented candidate. Review approval only routes into measurement — keep/discard is decided strictly by the measured-aggregate condition chain.",
    },
  );
}
