import { DEFAULT_VARIANT_DOCTRINE, LEFTOVER_DOCTRINE, REVIEW_VERDICT_DOCTRINE } from "@ctx-traits/agents";
...
import { reviewRubric } from "../resource.ts";

const REVIEW_EXTRA_INSTRUCTION = `Report "recurrence-rounds" only when a carried blocker has the same still-open step statuses with zero delta from the prior verdict: 1 for a first-raised blocker, 0 when blockers is empty, and reset to 1 whenever progress changes a carried step status.`;
