...
import { annotations, plan, planAnswer, task } from "../data.ts";

// The plan's shape (scope, stages, approach, risks) and its doctrine — an
// actionable plan for this run, honoring a task that declares itself atomic —
// live in the plan slot's schema descriptions, not here. The owner's plan-gate
// corrections, when present, are binding input to the redraft; the prior draft
// is not re-read — the corrections carry the exact plan text they were made on.
...
// The tree lane: the owner's annotations are the task, so the plan is drafted
// from them and the annotated code, not from a task file.
