...
import { doneCriteria, durationTarget, grounding, nextKey, slicePlan, taskInput, workItems } from "../data.ts";
  ...
  flow.loop("Planning", (loop) => {
    ...
    agent.prompt("Split the work into slices", {
      ...
      input: input.prompt`
            ...
            Assign final board keys now, starting at ${nextKey}: the first slice's charter takes that key, the next slice the following number, and so on; children are "<charter-key>.1", "<charter-key>.2", ... in dependency order (ordinals continue ".10", ".11", ... — never zero-padded). Keys are final — no placeholders.
            Plan at most eight slices — consolidate related work items into one slice rather than exceeding the cap. Every work item MUST appear in at least one slice's covers; a slice may own several related items. Size every child task to roughly ${durationTarget} of focused agent work; split anything larger into more children. A slice owns as many children as its scope needs — there is no fixed child count; split a slice only when its children stop serving one coherent outcome. Order slices and children so each depends only on earlier keys, and record real dependencies explicitly in each task's depends-on.
