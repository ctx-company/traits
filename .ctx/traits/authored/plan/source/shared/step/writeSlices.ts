    ...
    agent.prompt("Write the slice", {
      input: input.prompt(
        `Write exactly one slice of the plan to the board — {slice} — as TaskDocument TOML files in .internal/tasks/ (create the directory if missing). Do not write any other slice's files; other slices run in their own frames.
            ...
            The slice's tasks list is always EMPTY under the single-parent flow (owner ruling 2026-09-01): write exactly one parent task file under the slice's key — no charter language, no children; decomposition is architect's, later. If the plan somehow carries children, write only the parent and note the dropped children in the receipt. Keys stay symbolic (KEY-prefixed) in file names and contents alike — the renumber step assigns real numbers at the end of the run. Stamp raised = {raised-date} in every file. Ground each task's content/scope/validation in the work items {work-items}, the done criteria {done-criteria}, and the grounding notes {grounding} — fold in the affected modules or areas, applicable constraints, and existing validation gates that task needs so it stands alone. Defer execution-level decisions to architect; never prescribe exact symbols, signatures, edits, or edit sequencing. Derive each task's [[checks]] from its done criteria per the doctrine, sourcing commands from the grounding notes' validation gates.
