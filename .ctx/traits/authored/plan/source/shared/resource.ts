...
- Decompose along the WORK, never the stack: a charter (or slice) is keyed to a work item, and one work item that spans many layers is ONE charter whose children may cross layers — schema, authoring surface, runtime, and their tests can live in one child when that is the coherent cut. More charters than work items is the tell that layers, not work, did the slicing.
- Ceremony is a defect in every form: never a define-first or architecture-only task (the contract belongs in the charter's own body), never an audit-only or parity-only task (verification belongs in the validation and [[checks]] of the task that changed the behavior), and never a task whose body is only running gates, tests, or checks (gates belong in the [[checks]] of the task that changed the code).
- Size SYMMETRICALLY to the run's requested duration target (default: roughly 10-15 minutes of focused agent work): split anything materially larger into more tasks, and merge anything materially smaller into a sibling.
