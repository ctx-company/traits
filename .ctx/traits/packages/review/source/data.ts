...
const treeStatusSchema = schema.object("review-tree-status", {
    ok: schema.field(schema.boolean(), { description: "True when the reviewed tree has no uncommitted changes after the review step ran." }),
...
export const reviewDocumentReport = port.output.text({
    ...
    description: "The rendered human review document. Absent when preflight parked on an unresolvable range or an empty diff.",
    optional: true,
    value: reviewDocument,
});
export const treeStatusReport = port.output.of(treeStatusSchema, {
    id: "tree-status-report",
    description: "Whether the reviewed tree came out clean. Absent when preflight parked on an unresolvable range or an empty diff.",
    optional: true,
    value: treeStatus,
});
