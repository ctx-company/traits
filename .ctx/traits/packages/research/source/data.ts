...
export const researchStreamSchema: SchemaHandle = schema.object(
    "research-stream",
    {
        ...
        focus: schema.field(schema.text(), { description: "The complete, non-overlapping research question this stream owns." }),
...
export const streamFindingSchema: SchemaHandle = schema.object(
    "stream-finding",
    {
        ...
        summary: schema.field(schema.text(), { description: "Cited findings for this stream: claims, sources, and any counterevidence encountered." }),
    },
...
export const triangulation = slot.text({
    id: "triangulation",
    description: "Cross-stream triangulation: where independent streams agree, where they conflict, source-quality ratings, and remaining gaps.",
});
...
export const workSummary = slot.text({
    id: "work-summary",
    description: "Worker's cumulative account of the delivered research state: files written, how citations were verified, open concerns.",
});

export const verdict1 = slot({ id: "review-verdict-1", schema: reviewVerdictSchema, description: "First reviewer's verdict for the current research state." });
export const verdict2 = slot({ id: "review-verdict-2", schema: reviewVerdictSchema, description: "Second, independent reviewer's verdict for the current research state." });
...
export const topicSlug = slot.text({
    id: "topic-slug",
    description: "Deterministic kebab-case slug for the topic, derived by a command step — never agent prose — so every deliverable path is predictable.",
});
export const reportPath = slot.text({
    id: "report-path",
    description: "Deterministic repo-relative path to this variant's primary deliverable, derived by a command step from port:output-dir and topic-slug.",
});
...
export const commitReport = port.output.text({
    id: "commit-report",
    description: "Final commit evidence from the git commit command step. Absent when the clean-tree gate skipped the commit tail entirely.",
    optional: true,
    value: commitOutput,
});
...
export const researchReportPort = port.output.text({
    ...
    description: "Final work summary for the completed research: what was written, how citations were verified, open concerns.",
    optional: true,
    value: workSummary,
    format: "markdown",
});
