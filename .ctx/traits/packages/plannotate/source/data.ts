...
// Caller-supplied assignment text: the task leaves the assignment's source
// implicit, so this is authored as an explicit input port. Flagged in the PR
// for owner confirmation — a different source (e.g. an annotation tool run,
// mirroring implement-guided) may be preferred.
...
const verdict = cdkSlot({ id: "review-verdict", schema: schema.reviewVerdict, description: "The reviewer's verdict for the current round, plus its owner-facing briefing." });
const ownerBriefing = cdkSlot.text({ id: "owner-briefing", description: "The current round's briefing, lifted flat from review-verdict.briefing for the file-writing command step (a command step interpolates only flat slots, never a nested field)." });
...
const brief = cdkSlot({ id: "brief", schema: schema.brief, description: "The scribe's final brief: slug plus markdown, before it is written to .internal/briefs/." });
