...
const quickReviewText = prompt.text`
    ...
    ${reviewDiff} lists every file changed this round with its insertion/deletion counts — an index, not the change. Open whatever it points at with your own tools; it omits untracked files. ${repoGatesPassed} is this round's gate verdict; false is grounds for a blocker on its own — UNLESS its timed-out field is true, which is a repo condition (the ceiling is fixed in the trait), never the worker's blocker, and never grounds for a blocker on its own.
