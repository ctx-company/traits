...
const gatedProduceText = input.prompt`
    ...
    An owner decision may be attached: it is the owner's gate verdict on a previous round's briefing (denied or dismissed — an approved one would already have ended the loop), and its annotation text is the owner's exact words on what must change. The reviewer records an unsatisfied annotation as a typed blocker; read the annotation itself as the authoritative statement of what the owner wants.
...
const gatedReviewText = input.prompt`
    ...
    If an owner decision is attached to this frame, it is the owner's verdict on a previous round's briefing (denied or dismissed, since an approved one would already have ended the loop): treat any annotation it carries as an unsatisfied BLOCKER, never advisory, until the work visibly addresses it — record it as a typed blocker like any other, and carry it forward under the same verbatim rules.
    ...
    () => {
        ...
        step.command("Annotate the plan (plannotator, plan mode)", {
            ...
            argv: [
                "sh",
                "-c",
                'printf %s "$1" | jq -n --arg plan "$(cat)" \'{hook_event_name: "PreToolUse", tool_name: "ExitPlanMode", tool_input: {plan: $plan}}\' | PLANNOTATOR_ORIGIN=implement-gated plannotator',
                "sh",
                "{slot:draft}",
            ],
        ...
        flow.loop("Building", (loop) => {
            ...
            flow.when(
                ...
                () => {
                    ...
                    // lands in the output slot clean.
