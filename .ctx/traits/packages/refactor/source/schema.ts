...
import type { SchemaHandle, SchemaObjectField, SlotHandle } from "@ctx-traits/cdk";
import { schema, slot } from "@ctx-traits/cdk";

export function reviewVerdictSchema(variantId: string, extraFields: Record<string, SchemaObjectField> = {}): SchemaHandle {
    return schema.object(
        `review-verdict-${variantId}`,
        {
...
export function verdictSlot(id: string, reviewerLabel: string, schemaHandle: SchemaHandle): SlotHandle {
    return slot({ id, schema: schemaHandle, description: `${reviewerLabel}'s refinement verdict for the current refactored state.` });
}
