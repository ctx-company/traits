...
Graph with lockfile _semantics_ are proposed and benchmarked, not distributed
...
server. It says nothing about how the _skill itself_ is authored, typed,
...
[^1]:
    claim-gate row `package/version`, implemented/source-approved: "canonical
...
[^2]:
    same claim-gate row as [^1] (`package/version`) — version metadata is part
    of the same typed identity.

[^3]:
    claim-gate row `check`, implemented/source-approved: "check combines
...
[^4]:
    the shipped claim-gate row `audit` covers only direct hidden-content and
...
[^5]:
    claim-gate rows `check` and `diff`, implemented/source-approved: "diff
...
[^6]:
    claim-gate row `multi-agent/multi-harness runtime` (`runtime_claim_row()`
...
[^7]:
    claim-gate row `render/export`, implemented/source-approved: "render/export
...
[^c1]:
    Varun Pratap Bhardwaj, "Formal Analysis and Supply Chain Security for
    Agentic AI Skills" (SkillFortify), arXiv:2603.00195, 2026-02-27.

[^c2]:
    Changguo Jia, Tianqi Zhao, Runzhi He, Minghui Zhou, "Skills Are Not
...
[^c5]:
    Anthropic Agent Skills documentation (`SKILL.md` format), Claude Docs
    "Agent Skills".
