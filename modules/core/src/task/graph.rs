...
pub fn unmet_dependencies(documents: &BTreeMap<String, TaskDocument>, key: &str) -> Vec<String> {
    ...
    document
        ...
        .filter(|dependency| {
            !matches!(
                documents
                    .get(*dependency)
                    .map(|_| derived_status(documents, dependency)),
...
mod tests {
    ...
    fn unmet_dependencies_preserve_declared_order_and_dangling_keys() {
        ...
        dependent.relations.depends_on =
            vec!["0002".to_string(), "9999".to_string(), "0001".to_string()];
        ...
        assert_eq!(unmet_dependencies(&documents, "0003"), vec!["0002", "9999"]);
