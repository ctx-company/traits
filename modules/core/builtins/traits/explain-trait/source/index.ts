...
const receiptDigest = port.input.text({ id: "receipt-digest", description: "Digest of the check receipt the scaffold is grounded in." });
const scaffold = port.input.text({ id: "scaffold", description: "Deterministic explain-scaffold JSON text — the sole grounding evidence." });
...
const section = schema.object(
    "explain-section",
    { title: schema.text(), "receipt-section": schema.text(), "receipt-fact": schema.text(), "construct-ref": schema.text(), anchor },
...
const narrationPort = port.output.of({ id: "explanation", schema: ref.schema("explain-scaffold"), description: "Generated advisory explanation, echoing its grounding scaffold verbatim.", value: narrationOutput });
...
export default trait("explain-trait", {
    ...
    port: narrationPort,
    procedure: procedure({
        description: "Generate one untrusted plain-language narration strictly grounded in the supplied deterministic explain scaffold.",
