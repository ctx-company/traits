import { agent, condition, dependency, input, operation, port, procedure, ref, schema, sequence, slot, trait } from "@ctx-traits/cdk";
...
const traitId = port.input.text({ id: "trait-id", description: "Trait id the imported package must keep; also keys the round evaluator's scratch package and the persisted baseline it converges against." });
...
const roundReportSchema = schema.object(
    "round-report",
    {
        rung: schema.field(schema.text(), { description: "The last rung evaluated: the failing rung, or the ladder's final rung once converged." }),
        converged: schema.field(schema.boolean(), { description: "Whether the candidate cleared every rung." }),
        diagnostics: schema.field(schema.any(), { required: false, description: "Typed diagnostics from the failing rung; empty once converged." }),
    },
