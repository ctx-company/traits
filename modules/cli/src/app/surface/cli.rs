...
pub enum TraitsCommand {
    ...
    /// Install one npm-transport trait package into this project.
    ///
    ...
    Install {
        ...
        /// Install into the per-machine global tier
    ...
    /// Superseded by `ctx traits dependency remove`; kept as a hidden alias for
    /// one release (P567).
    #[command(hide = true)]
    Remove {
    ...
    /// dependency.
    /// Superseded by `ctx traits dependency outdated`; kept as a hidden alias for
    /// one release (P567).
    #[command(hide = true)]
    Outdated {
        /// Emit structured JSON.
    ...
    /// Publish exactly one ready trait package to npm without changing source files.
    /// Superseded by `ctx traits dependency publish`; kept as a hidden alias
    /// for one release (P567).
    #[command(hide = true)]
    Publish {
