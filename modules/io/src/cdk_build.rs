...
fn resolve_trait_alias(
    ...
    let manifest_text = std::fs::read_to_string(package_json_path.as_std_path()).map_err(|_| {
        crate::environment::Error::Process {
            ...
            timed_out: false,
            message: format!(
                "CDK alias import \"#{alias}\" in {importer} requires {package_json_path} \
                 (generated by ctx build from the trait manifest) to declare \"imports\": \
                 {{ \"{TRAIT_ALIAS_KEY}\": \"{TRAIT_ALIAS_VALUE}\" }} (Node subpath imports, \
                 Node >=22), but the manifest could not be read — run a build to regenerate it"
            ),
        }
    })?;
    ...
    if mapped_value != Some(TRAIT_ALIAS_VALUE) {
        return Err(crate::environment::Error::Process {
            ...
            message: format!(
                "CDK alias import \"#{alias}\" in {importer} requires {package_json_path} \
                 (generated by ctx build from the trait manifest) to declare \"imports\": \
                 {{ \"{TRAIT_ALIAS_KEY}\": \"{TRAIT_ALIAS_VALUE}\" }} exactly (Node subpath \
                 imports, Node >=22), found {mapped_value:?} — run a build to regenerate it"
            ),
        }
        .into());
    }
...
/// Build the `package.json` value ctx build owns for a trait package rooted
/// at `package_root`: every field is derived from `manifest`, never hand
/// authored. `name` prefers `[publish].name`, falling back to
/// `@ctx-traits/<id>`. `dependencies` is a verbatim copy of
/// `[authoring-dependencies]` (0170's territory is *how* those get pinned,
/// not this table shape). `files` lists exactly the package-root entries a
/// consumer needs; nothing else is published.
pub fn generated_package_json_value(
    manifest: &ctx_traits_core::manifest::PackageManifest,
) -> serde_json::Value {
    let name = manifest
        .publish
        .as_ref()
    ...
    let dependencies: serde_json::Map<String, serde_json::Value> = manifest
        .authoring_dependencies
        .iter()
        .map(|(alias, spec)| (alias.clone(), serde_json::Value::String(spec.clone())))
        .collect();
    serde_json::json!({
        "name": name,
        "version": manifest.package.version,
        "type": "module",
        "imports": { TRAIT_ALIAS_KEY: TRAIT_ALIAS_VALUE },
...
/// Render [`generated_package_json_value`] as the exact bytes ctx build
/// writes: `serde_json`'s map/struct field order (source order, not
/// alpha-sorted) pretty-printed with a trailing newline, so the same
/// manifest always produces byte-identical output — the invariant
/// [`write_generated_package_json`]'s overwrite-on-build and
/// check's divergence comparison both depend on.
pub fn generated_package_json_text(
    manifest: &ctx_traits_core::manifest::PackageManifest,
) -> String {
    let value = generated_package_json_value(manifest);
    // Every field is a plain string/table built above; this can never fail.
    let mut text =
        serde_json::to_string_pretty(&value).expect("generated package.json is valid JSON");
    text.push('\n');
    text
}

/// Path package.json is generated at for a package rooted at `package_root`.
pub fn generated_package_json_path(package_root: &Utf8Path) -> Utf8PathBuf {
    package_root.join("package.json")
}

/// Write the build-owned `package.json` for the package rooted at
/// `package_root`, unconditionally overwriting whatever was there — hand
/// edits never survive a build. Byte-stable: the same manifest always
/// produces the same bytes, so `check`'s divergence comparison
/// (`report_check.rs`) is a plain string comparison against this same
/// rendering.
pub fn write_generated_package_json(
    package_root: &Utf8Path,
    manifest: &ctx_traits_core::manifest::PackageManifest,
) -> crate::Result<()> {
    let text = generated_package_json_text(manifest);
    crate::write::write_build_output(&generated_package_json_path(package_root), &text)
}
