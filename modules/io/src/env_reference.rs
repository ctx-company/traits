...
/// Forces a completed debug drive through the ordinary killed terminal path.
/// Compiled out of release builds; used only by the terminal-reclaim proof.
#[cfg(debug_assertions)]
pub const TESTHOOK_FORCE_KILLED_OUTCOME: &str = "CTX_INTERNAL_TESTHOOK_FORCE_KILLED_OUTCOME";
...
#[cfg(debug_assertions)]
fn testhook_env_reference() -> [EnvVarDoc; 10] {
    [
        EnvVarDoc {
        ...
        EnvVarDoc {
            name: TESTHOOK_FORCE_KILLED_OUTCOME,
            contract: "Terminal-reclaim proof injection: routes a completed drive through its ordinary killed terminal outcome before outcome recording. A no-op unless set. Absent from release builds.",
            kind: EnvVarKind::DebugOnlyTestHook,
        },
        EnvVarDoc {
